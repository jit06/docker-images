<?php
	...
	**/
	...
	$default_temp_folder=$default_private.'/tmp'; # temp folder path 
	...
	$allow_shared_folder_RSS_feed=false;							# Visitor can access to the rss feed of a shared folder 
	$allow_shared_folder_JSON_feed=false;						# Visitor can access to the json feed of a shared folder 
	...
	$max_length=getenv('BOZON_MAX_UPLOAD');						// Mo (see php.ini if changes doesn't work [post_max_size / upload_max_filesize])

	#############################################################
	# Profiles config
	#############################################################
	$default_profile_folder_max_size=getenv('BOZON_MAX_PROFILE_SIZE'); 						# false = infinite or specify in MB
	
	#############################################################
	# Misc
	#############################################################
	$cron_security_string=getenv('BOZON_CRON_SECURITY');					# basic security access: change it. cron url is: www.myserver.com/?cron_update=aQw1zSx2eDc3rFv4
